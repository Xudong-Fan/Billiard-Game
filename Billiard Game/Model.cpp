...
bool Model3D::load_Model(const std::string& path){
	clear();
	...
	static Assimp::Importer assimp;
	const aiScene* scene = assimp.ReadFile(path, aiProcessPreset_TargetRealtime_Quality);
	if (scene == NULL){
...
void Model3D::importTextures(const aiScene* scene, const std::string& path){
	...
	for (unsigned int i = 0; i < scene->mNumMaterials; i++){
		...
		if (aiMat->GetTexture(aiTextureType_DIFFUSE, 0, &file) == aiReturn_SUCCESS){
			...
			if (tex->load_Texture(parsingURL(path, PARSING_DIRECTORY) + "/" +
				parsingURL(file.data, PARSING_DIRECTORY | PARSING_FILE_NAME | PARSING_FILE_EXTENSION))){
				textures[file.data] = tex;
				continue;
			}
			if (tex->load_Texture(parsingURL(path, PARSING_DIRECTORY | PARSING_FILE_NAME) + "/" +
...
void Model3D::draw(const Node* node, const glm::mat4& preMatrix){
	...
	for (const Mesh* mes : node->content){
		...
		if (tex != NULL){
			...
			glBindTexture(GL_TEXTURE_2D, tex->get_Texture_Unit());
